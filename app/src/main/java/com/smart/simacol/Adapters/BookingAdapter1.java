...
public class BookingAdapter1 extends FirebaseRecyclerAdapter<Booking, BookingAdapter1.BookingHolder> {
    ...
    Context context;
    public BookingAdapter1(FirebaseRecyclerOptions<Booking> options) {
        super(options);
    }
    ...
    protected void onBindViewHolder(@NonNull final BookingHolder holder, int position, @NonNull Booking model) {
        ...
        uid = user.getUid();
        //deklarasi variabel
        bookRef = FirebaseDatabase.getInstance().getReference().child("USER-BOOKING/"+uid+"/room1");
        bookAllRef = FirebaseDatabase.getInstance().getReference().child("BOOKING/room1");
